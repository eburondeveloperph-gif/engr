...
import { Camera, Upload, CheckCircle, Loader2, DollarSign, TrendingUp } from 'lucide-react';
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        // Using gemini-3-pro-preview for complex image analysis as requested
    ...
    <div className="h-full flex flex-col bg-slate-50 overflow-y-auto no-scrollbar pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div className="text-slate-500 text-xs uppercase tracking-wider mb-1">Total Revenue</div>
          <div className="text-2xl font-bold text-green-600 flex items-center gap-1">
             <DollarSign size={20} /> {totalSales.toLocaleString()}
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div className="text-slate-500 text-xs uppercase tracking-wider mb-1">Total Expenses</div>
